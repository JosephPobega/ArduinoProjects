...
Modified by Brendan Harris / Joseph Pobega (July 8, 2022).
*/
...
void loop(){
  long duration;
  float distanceCent, distanceInch; // Declare centimeter and inch variable
  ...
  distanceCent = (duration/2) / 29.1; // cm formula
  distanceInch = (distanceCent)/2.54; // inches formula
  LCD.print("                "); //Print blanks to clear the row
  LCD.setCursor(0,0);   //Set Cursor again to first column of first row
  LCD.print(distanceCent);
  LCD.print(" cm");
